...
def image_process(image, width, height, opt=[]):
    ...
    for effect in opt:
        ...
        if effect == 'into':
            ...
            if scale > 1:
                image = image.resize((int(width), int(width/scale)), Image.ANTIALIAS)
            else:
                image = image.resize((int(height*scale), int(height)), Image.ANTIALIAS)
            ...
            if scale > 1:
                ...
                image = image.crop(
                    [int(1.0 * w / 2 - 1.0 * w * scale / 2), 0, int(1.0 * w / 2 + 1.0 * w * scale / 2), h - 1])
            ...
            if len(opt) == 4:
                ...
                blank = Image.new('RGBA', [width, height], (int(opt[1]), int(opt[2]), int(opt[3]), int(opt[4])))
            else:
...
def thumbnail(file, size='200;200'):
    """ 
      fix, blank, wtm
    """
    correct_suff = ['fix', 'blank', 'wtm', 'resize', 'into']
    ...
    for su in opt:
        if su.split(':')[0] in correct_suff: suff += unicode(su.split(':')[0])
...
def xendor_dummy(line_size, size='200;200'):
    ...
    try:
        ...
        miniature = 'xdummy_' + line_size + '_ ' + color.replace('#', '') + '_ ' + background.replace('#',
                                                                                                      '') + '_ ' + sizex + '.png'
    except:
